...
class PredictViewModel(
    ...
    sealed interface Intent {
        ...
        data class SetMultiSelect(val betId: String, val selectedOptionIds: List<String>) : Intent
        data class SetOverUnder(val betId: String, val over: Boolean) : Intent
    ...
    override fun onIntent(intent: Intent) {
        when (intent) {
            ...
            is Intent.SetMultiSelect -> update {
                it.copy(drafts = it.drafts + (intent.betId to PredictionPayload.MultiSelect(intent.selectedOptionIds)))
            }
            is Intent.SetOverUnder -> update {
    ...
    private suspend fun submitNow() {
        ...
        // No uid yet — bootstrap an anonymous account so predictions reach the cloud
        // (creating the player doc that makes the challenge appear in the list).
        // Fall back to local-only if the anonymous sign-in fails (e.g. offline).
        if (isLocalOnly) {
            val bootstrapped = auth.continueAsGuest()
